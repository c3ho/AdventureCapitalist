const { addShop, getShop, getShops } = require('../utils/storage')

class Shop {

    constructor(shopNumber, name, amount = 0, revenue, cost = 0, coefficient = 0, timeOut = 0, revMultiplier = 1, baseTimerMultiplier = 1, available = true){
    ...
    static async byId(id) {
        ...
        return new Shop(parseInt(data.shopNumber), data.name, parseInt(data.amount), parseInt(data.revenue), parseInt(data.cost), parseFloat(data.coefficient), parseInt(data.timeout), parseFloat(data.revMultiplier), parseFloat(data.baseTimerMultiplier), data.available === 'true')
    }
