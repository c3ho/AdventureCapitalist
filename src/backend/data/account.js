...
class Account {
    ...
    constructor(cash = 0, shops = [], managers = []) {
        ...
        if (shops.length === 0) {
            ...
            shop = new Shop(6, 'Hockey Team', 0, 7464960, 14929920, 1.10, 384)
            newShops.push(shop)
            ...
            shop = new Shop (9, 'Oil Company', 0, 29668737024, 25798901760, 1.07, 36864)
            newShops.push(shop)
    ...
    async purchaseShop(shopName, amount) {
        ...
        if (this.minShopAmount >= 25 && this.minShopAmount < 50){
            ...
            this.shops.map(shop => shop.baseTimerMultiplier = shop.baseTimerMultiplier * this.timerMultiplier)
        ...
        if (this.minShopAmount >= 500) {
            this.revenueMultiplier = Math.pow((Math.floor(this.minShopAmount - 500 / 100) + 1), 2)
        }
        await this.save();
    }
    
    hireManager(managerName) {
        let index = this.managers.findIndex((manager) => manager.name === managerName);
        // index not found
        if (index < 0) {
            return;
        }
